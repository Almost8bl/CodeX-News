document.addEventListener('DOMContentLoaded', () => {
    ...
    async function initialize() {
        ...
        try {
            const response = await fetch('https://api.jsonbin.io/v3/b/687fde5e7b4b8670d8a59276'); // تأكد من هذا الرابط!
            ...
            // فحص بنية JSON لاستخراج التقارير/المقالات
            // تم تكييف هذا الجزء للتعامل مع المفتاح 'reports' أو 'articles' أو مصفوفة مباشرة
            ...
            } else if (parsedData && parsedData.record && Array.isArray(parsedData.record.articles)) {
                reportsData = parsedData.record.articles; // في حال كانت البيانات تحتوي على 'articles' بدلاً من 'reports'
            } else if (parsedData && Array.isArray(parsedData.articles)) {
                reportsData = parsedData.articles;
            } else if (Array.isArray(parsedData)) {
                reportsData = parsedData;
            } else {
                console.warn("JSON data does not contain a 'reports' or 'articles' array as expected.", parsedData);
                reportsData = [];
            }

            console.log("Reports/Articles data fetched successfully. Total items:", reportsData.length);

            if (reportsData.length > 0) {
                populateSidebar(reportsData);
                // عرض أول مقال/تقرير افتراضيًا
                displayArticle(reportsData[0].id);
            } else {
                reportsListNav.innerHTML = `
                    <div class="text-center text-gray-500 py-4 px-2">
                        <p class="text-lg">لا توجد تقارير/مقالات لعرضها حاليًا.</p>
                    </div>
                `;
            }
            ...
            mainContent.innerHTML = `
                <div class="placeholder text-center text-red-600 p-8 bg-red-50 rounded-lg shadow-md">
                    ...
                    <p class="text-lg text-gray-700">فشل تحميل سجلات التقارير/المقالات. يرجى التحقق من اتصالك بالإنترنت والمحاولة لاحقاً.</p>
    ...
    // --- 3. Display Article in Main Content (تعديلات على هذه الدالة فقط) ---
    function displayArticle(articleId) {
        const article = reportsData.find(item => item.id === articleId); // البحث في reportsData
        if (!article) {
            mainContent.innerHTML = `
                <div class="placeholder text-center text-gray-600 p-8 bg-yellow-50 rounded-lg shadow-md">
                    ...
                    <p class="text-lg text-gray-700">عذراً، لم نتمكن من العثور على المقال المطلوب بمعرف: <strong>${articleId}</strong>.</p>
                </div>
            `;
        ...
        // تم تكييفه ليعكس "المقال" بدلاً من "التقرير" وافتراض وجوده في بيانات المقال
